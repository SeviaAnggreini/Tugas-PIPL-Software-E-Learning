...
foreach ($sheet as $data){
	...
	$no_telp		= $data['G'];
	$email			= $data['H'];
	$password		= sha1("dosenskr76");
	$hak_akses		= "Dosen";

	if($nip == "NIP" && $nama_dosen == "Nama Dosen" && $jenis_kelamin == "Jenis Kelamin" && $tempat_lahir == "Tempat Lahir" && $tanggal_lahir == "Tanggal Lahir" && $alamat == "Alamat" && $no_telp == "No Telp" && $email == "Email"){
		continue;
	} else if($nip != "" && $nama_dosen != "" && $jenis_kelamin != "" && $tempat_lahir != "" && $tanggal_lahir != "" && $alamat != "" && $no_telp != "" && $email != ""){
...
if($berhasil == $jumlah_baris){
		...
		<script type="text/javascript">
			...
			setTimeout("location.href='?page=akun_dosen&aksi=import_siswa'", 1000);
		</script>
		<?php
	}
?>